...
// WARNING: Initializing Gemini API in the browser exposes your API key.
// Ensure your VITE_GEMINI_API_KEY is restricted to specific domains.
const getAi = () => {
...
const handleApiError = (error: any, defaultMsg: string) => {
  ...
  if (errorMsg.includes("429") || errorMsg.includes("RESOURCE_EXHAUSTED")) {
    throw new Error("You have exceeded your Gemini API usage quota. Please check your API key billing details or try again later.");
  }
  if (errorMsg.includes("503") || errorMsg.includes("UNAVAILABLE")) {
    throw new Error("The AI is currently experiencing high demand. Please try again in a few moments.");
  }
  if (errorMsg.includes("400") && errorMsg.includes("API key not valid")) {
    throw new Error("Your Gemini API key is invalid. Please check your settings.");
  }
...
export const generateWikiFromFiles = async (files: FileData[]): Promise<WikiData> => {
  try {
    ...
    console.error(error);
    handleApiError(error, "Failed to generate wiki from files");
    throw error;
  }
};
...
export const generateWikiFromTopic = async (topic: string): Promise<WikiData> => {
  try {
    ...
    console.error(error);
    handleApiError(error, "Failed to generate wiki from topic");
    throw error;
  }
};
